...
class ApiCaller:
    ...
    def __init__(self, api_key, api_secret, server, phantom):
    ...
    def call(self, request_handler, headers={'User-agent': 'VxPhantom'}, verify_server=False):
        ...
        request_url = self.server + self.endpoint_url
        ...
        self.api_response = caller_function(request_url, data=self.data, params=self.params, files=self.files, headers=headers, verify=verify_server, auth=HTTPBasicAuth(self.api_key, self.api_secret))
    ...
    def get_response_msg_success_nature(self):
        if self.api_response is None:
            raise Exception('It\'s not possible to define api msg nature since API was not called.')
